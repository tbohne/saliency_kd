...
PROMPT_APPENDIX = "\n\nDescribe them at the same granularity as the examples above. Only then match each description to the best fitting case. Sometimes several might match in a way, it is important then to select the one that matches more precisely. Important: only match things that actually reasonably match; it is crucial to also identify signals that don't match any class. There also might be duplicates, i.e., more than one signal matching the same class. When the class description mentions that some values are kept 'for a while' or something similar, it means that the peak looks more like a rectangle (many steps with roughly same val, something like 20% of the signal length), i.e., clearly distinct from a triangular sort of peak (few points with roughly the same val, perhaps around up to 5% of the signal length) where it goes up and down again. Finally, note that the peaks mentioned in the descriptions above are only once that are very significant ones, i.e., roughly around a third of the signal's value range or more."
